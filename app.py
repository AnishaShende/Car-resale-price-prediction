...
# with open('rf_rscv.pkl', 'rb') as f:
#     pickle_model = pickle.load(f)

# # Save the model using joblib
# joblib.dump(pickle_model, 'rf_rscv.joblib')

# # Load the joblib model
# rf_rscv_joblib = joblib.load('rf_rscv.joblib')
...
def model():
	...
	if request.method=='POST' and form.validate():
		...
		value_rf = random_forest_model.predict(inputs)[0]
		# value_rf = round(value_rf, 2)
		value_max = value_rf * (1.025)
		value_min = value_rf * (0.975)
		print(f"value_rf = {value_rf}, value_min = {round(value_min, 2)}, value_max = {round(value_max, 2)}, dis_dict = {dis_dict}")
